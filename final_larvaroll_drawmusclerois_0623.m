%% larva_roll_muscle_ROIs - revised to final 06/23
...
%UPDATE: 06/23 to work with Matlab 2023a functions
%Grueber Lab
%Columbia University
    ...
    for fi = 1:length(filenames)
        if contains(filenames(fi).name,'roll')
            if contains(filenames(fi).name,'gcamp')
                ...
                for a = 1 : frames
                    i_tempgreen(:,:,a) = imread(tiffnamegreen, a);%./40; %adjust divide by 40 for larva10 bc expression diffs
                end
...
function [newtraces,newrois,musclename] = runseg(i_tempred,i_tempgreen,rois,fixframes,imagec,newtraces,newrois)
    ...
    if sum(~cellfun(@isempty,newrois))>0
        fr = fixframes(1);
    elseif sum(~cellfun(@isempty,newrois))==0
        fr = input('Start Frame? enter = 1, # = skip to #'); %draw or skip
            if isempty(fr)
                fr = 1;
            end
    end
   
    drawstop = fixframes(end);
    while fr <= drawstop
%        fr = foi;
        ...
        drawnow = input('Draw New ROI? enter = yes, 0 = no, 1 = keep old ROI, 2 = finishup'); %draw or skip
        if isempty(drawnow) || drawnow == 1
            ...
            newtraces(fr,1) = mean(wopunctagreen_90, 'omitnan');
            newtraces(fr,2) = mean(i_segred(newmred), 'omitnan');
            newtraces(fr,3) = mean(wopunctared_90, 'omitnan'); 

            %calculate the ratio of the ROI, then take the mean
            roiratio = (wopunctagreen_90./wopunctared_90);
            newtraces(fr,4) = mean(roiratio,'omitnan');
        ...
        elseif drawnow == 0
            ...
            newtraces(fr,4) = 0;
        elseif drawnow == 2
            %save 0's for the rest of the frames and stop the loop
            newtraces(fr:drawstop,1) = 0;
            newtraces(fr:drawstop,2) = 0;
            newtraces(fr:drawstop,3) = 0; 
            newtraces(fr:drawstop,4) = 0;
        end
        fr = fr + 1;
        if drawnow == 2
            fr = drawstop+1;
